...
namespace Argent::Component::Animation
{
	...
	{
		AnimationPlayer(std::vector<Resource::Animation::AnimationClip> clips);
		...
		const Resource::Animation::AnimationClip::Keyframe& GetCurrentKeyFrame() const { return clips.at(clipIndex).sequence.at(static_cast<int>(frameIndex)); }
